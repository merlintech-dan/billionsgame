...
function draw() {
  ...
  snake.forEach((seg, i) => {
    if (i === 0) {
      ...
      drawRect(seg.x, seg.y, '#007BFF');
    }
  });

  // Fruit with pop animation
  if (fruitAnimating) {
    fruitPopFrame++;
...
function saveScore(name, score) {
  ...
  localStorage.setItem('snakeLeaderboard', JSON.stringify(leaderboard.slice(0, 5)));
}
...
function handleSwipe() {
  ...
  if (Math.abs(dx) > Math.abs(dy)) {
    if (dx > 30 && !isOpposite('right', direction)) nextDirection = 'right';
    else if (dx < -30 && !isOpposite('left', direction)) nextDirection = 'left';
  } else {
    if (dy > 30 && !isOpposite('down', direction)) nextDirection = 'down';
    else if (dy < -30 && !isOpposite('up', direction)) nextDirection = 'up';
  }
}

// Button Controls
upBtn.addEventListener('click', () => { if (!isOpposite('up', direction)) nextDirection = 'up'; });
downBtn.addEventListener('click', () => { if (!isOpposite('down', direction)) nextDirection = 'down'; });
leftBtn.addEventListener('click', () => { if (!isOpposite('left', direction)) nextDirection = 'left'; });
rightBtn.addEventListener('click', () => { if (!isOpposite('right', direction)) nextDirection = 'right'; });
